...
{ 
    ...
    HAL_NVIC_SetPriority(RTC_Alarm_IRQn, 2, 2); //设置中断优先级
	HAL_NVIC_EnableIRQ(RTC_Alarm_IRQn);		 //开启中断
}
//周期性唤醒定时器设置
...
{ 
    ...
    HAL_NVIC_SetPriority(RTC_WKUP_IRQn, 2, 2); //设置中断优先级
	HAL_NVIC_EnableIRQ(RTC_WKUP_IRQn);		 //开启中断
}
